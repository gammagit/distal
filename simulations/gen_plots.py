...
def plot_multpile_exp(results, ncat=6, columns=[1, 2, 3],\
                      labels=['No Augmentation', 'Translation', 'Trans + Rot + Scale'], model_name='vgg', learnconv=True):
    ### Plot Same and Diff at different stops
    # sns.set(style='whitegrid')
    set_plot_style()

    ### Create barplots in a FacetGrid
    g = sns.FacetGrid(results, col='expt', col_order=columns, sharex=False)
    g.map(sns.barplot, 'expt', 'avg_accu', 'condition', hue_order=['basis', 'v1', 'v2'])
    axes = np.array(g.axes.flat)
    color_bars(axes, get_colors()) # Set colors
    for i,ax in enumerate(axes):
        ax.set_xticks([-0.3, 0, 0.3])
        ax.set_xticklabels(['Basis', 'Rel', 'Cod'], rotation=0, position=[0, 0, 0])
    ...
    out_file = os.path.join('analysis', model_name + '_all_exp.png')
...
def plot_exp_results(results, ncat=4, labels=['Basis', 'D1', 'D2']):
    ...
    out_file = os.path.join('experiment', 'all_cats.png')
    fig.savefig(out_file)
    fig.savefig(out_file)
